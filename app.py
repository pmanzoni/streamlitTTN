# version PM - con página de estado de gateways
...
# Umbral en segundos para considerar un gateway como "inactivo"
GATEWAY_TIMEOUT_S = 300  # 5 minutos
...
# gateway_registry: dict keyed by gateway_id
# cada entrada guarda: last_seen (datetime), last_rssi, last_snr, lat, lon, alt,
#                      mensajes (contador), devices_seen (set → serializado como lista),
#                      rssi_history (lista últimos N valores)
...
if data:
    ...
    # Actualizar registro de gateways
    now = datetime.now()
    for gw in gateways:
        ...
        if gw_id not in st.session_state.gateway_registry:
            ...
            # Actualizar coordenadas si llegan
            if lat is not None:
                entry["lat"] = lat
                entry["lon"] = lon
                entry["alt"] = alt

    # Guardar paquete en historial
    paquete = {
...
# ============================================================
# PÁGINA 1: DASHBOARD (código original)
# ============================================================
if pagina == "📡 Dashboard":
    ...
    if data:
        ...
        if lilygo_lat is not None and lilygo_lon is not None:
            ...
            st.warning("⚠️ La LilyGO está transmitiendo, pero el GPS todavía no tiene FIX. Mostrando posición de respaldo...")
...
# ============================================================
    ...
    if not registry:
        ...
        # Ordenar: primero activos, luego inactivos, dentro de cada grupo por mensajes desc
        def sort_key(item):
            gw_id, v = item
            segundos = (now - v["last_seen"]).total_seconds()
            activo = segundos < GATEWAY_TIMEOUT_S
            return (not activo, -v["mensajes"])

        for gw_id, v in sorted(registry.items(), key=sort_key):
            segundos_inactivo = (now - v["last_seen"]).total_seconds()
            activo = segundos_inactivo < GATEWAY_TIMEOUT_S

            if activo:
                estado_label = "🟢 Activo"
                estado_color = "green"
                tiempo_label = f"Hace {int(segundos_inactivo)}s"
            else:
                estado_label = "🔴 Inactivo"
                estado_color = "red"
                mins = int(segundos_inactivo // 60)
            ...
            with st.container(border=True):
                ...
                m2.metric("RSSI actual", f"{v['last_rssi']} dBm" if v["last_rssi"] is not None else "N/A")
                m3.metric("SNR actual", f"{v['last_snr']}" if v["last_snr"] is not None else "N/A")
                m4.metric("Mensajes recibidos", v["mensajes"])
                m5.metric("Dispositivos vistos", len(v["devices_seen"]))

                # Dispositivos que han pasado por este gateway
                with st.expander("Ver dispositivos y RSSI histórico"):
                    st.write("**Dispositivos detectados:**")
                    st.write(", ".join(v["devices_seen"]) if v["devices_seen"] else "Ninguno")
        ...
        # --- Mapa de gateways con ubicación conocida ---
        map_rows = [r for r in tabla_rows if r["Lat"] is not None and r["Lon"] is not None]
        if map_rows:
            st.subheader("🗺️ Mapa de gateways")
            mapa_df = pd.DataFrame(map_rows).rename(columns={"Lat": "lat", "Lon": "lon"})
